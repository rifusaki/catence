...
export const DATASET_CATALOG: Record<Exclude<DatasetName, 'training_metrics'>, DatasetDefinition> = {
  ...
  swim_lengths: {
    name: 'swim_lengths', relation: 'swim_length_facts', description: 'Explicit provider-supplied swim-length facts. An empty result means lengths were not supplied; Catence does not infer them from activity samples. Per-length swolf may be null (not supplied by all providers); derive as duration_s + stroke_count. Per-length distance is typically null in Garmin FIT; use pool_length_m to compute pace (duration_s / pool_length_m * 100 for s/100m).', dateColumn: 'started_at_utc', provenanceColumns: ['activity_source_id', 'source', 'confidence', 'raw_object_hash'], permittedGroupings: ['provider', 'sport', 'source', 'confidence', 'is_rest'],
