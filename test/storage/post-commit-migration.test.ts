...
describe('post-commit migrations', () => {
  it('migrates a populated v9 training-metrics table before rebuilding its index', async () => {
    ...
    try {
      ...
      await connection.run('CREATE TABLE activity_sources (activity_source_id VARCHAR, activity_id VARCHAR)');
    ...
    try {
      expect(await database.rows("SELECT version FROM schema_migrations WHERE version = 10")).toEqual([{ version: 10 }]);
